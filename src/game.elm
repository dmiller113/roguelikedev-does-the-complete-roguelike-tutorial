...
initialPosDict =
    Dict.singleton 0 initialPos


initialDrawDict : Dict Int DrawInfo
initialDrawDict =
    Dict.singleton 0 initialDrawInfo


initialMovables : Dict Int Momentum
initialMovables =
    Dict.singleton 0 initialMomentum


initialPhysicals : PhysicalDict
initialPhysicals =
    Dict.singleton 0 { blocksMovement = True, blocksSight = False }
...
initF flags =
    ( { actor = { id = 0, name = "Player" }
      ...
      , fov = Set.empty
      , explored = Set.empty
      , initialSeedValue = flags.initialSeed
      , currentSeed = Random.initialSeed flags.initialSeed
      }
        ...
        Init ->
            let
                ( map, newPositions, newDrawables, physicals, newId, seed ) =
                    initMap model.nextAvailableId
                        (mapDimensions.x - 1)
                        (mapDimensions.y - 1)
                        model.components.positions
                        model.components.drawables
                    <|
                        RogueGenerator 80 24 model.currentSeed
                ...
                playerPosition =
                    Dict.get 0 newPositions |> Maybe.withDefault initialPos
            in
                ( { model
                    | state = GamePlay
                    , currentMap = map
                ...
                , Cmd.batch [ (createFov <| produceFoVMap physicals newPositions), computeFov { x = playerPosition.x, y = playerPosition.y, r = 10 } ]
                )
    ...
    case msg of
        Reset ->
            let
                ( imodel, cmd ) =
                    init
            in
                ( { imodel | initialSeedValue = model.initialSeedValue, currentSeed = model.currentSeed }, cmd )

        Tick newTime ->
            let
                di =
                    Maybe.withDefault initialDrawInfo <| Dict.get model.actor.id model.components.drawables
            in
        ...
        KeyDown code ->
            let
                ...
                movables =
                    updateMovables model.components.movables key model.actor.id

                di =
                    Maybe.withDefault initialDrawInfo <|
                        Dict.get model.actor.id model.components.drawables

                newPositions =
                    moveActors movables model.components.physicals model.components.positions

                pos =
                    Maybe.withDefault initialPos <| Dict.get model.actor.id newPositions

                newDrawables =
                    (Dict.union <| Dict.singleton model.actor.id { di | position = pos }) model.components.drawables
...
renderView fov explored dictDi =
    let
        actorDI =
            Maybe.withDefault initialDrawInfo <| Dict.get 0 dictDi

        actorString =
            drawInfoToString fov explored actorDI

        actorPos =
            actorDI.position

        posInt =
            (actorPos.x + actorPos.y * mapDimensions.x) * 31
    in
